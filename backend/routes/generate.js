...
router.post('/:id/outline', protect, async (req, res) => {
  try {
    ...
    // Configure model with strict JSON schema response
            ...
            sections: {
              ...
              items: {
                ...
                properties: {
                  heading: { type: 'STRING' },
                  level: { type: 'INTEGER' },
                  brief_intent: { type: 'STRING' }
                },
                required: ['heading', 'level', 'brief_intent']
              }
            }
          },
    ...
    const parsedOutline = JSON.parse(responseText);
...
router.post('/inline-edit', protect, async (req, res) => {
  try {
    ...
    const updatedText = result.response.text().trim();
